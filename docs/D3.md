...
| Branch stabile | `main` |
| Apiary/OpenAPI | Blueprint locale in `docs/apiary_blueprint.txt` e specifica OpenAPI in `docs/openapi.yaml`; resta da pubblicare su piattaforma esterna |
...
| PB19 | Specifica Apiary/OpenAPI | Media | Completato localmente | `docs/apiary_blueprint.txt`, `docs/openapi.yaml`; pubblicazione esterna non effettuata |
...
| SB04 | Implementare QR ufficiale su `/api/v1/qr` | Paolo Sarcletti | Done | `qrController.js`, `scambioQrService.js` |
...
Le decisioni prese durante lo sprint hanno portato a centralizzare la logica QR/scambio in `scambioQrService.js`, a considerare `/api/v1/qr` come flusso ufficiale e a esplicitare nella D3 i limiti di riproducibilita dei test automatici.
...
Decisione di review: il flusso ufficiale da presentare e `/api/v1/qr`. Gli endpoint `/api/v1/scambi` restano presenti solo come route legacy deprecate e rispondono con `410 Gone`.

La review ha inoltre confermato che tutte le funzionalita principali sono esposte tramite API REST, come richiesto dal template di milestone. La specifica Apiary/OpenAPI non era stata pubblicata nello Sprint 1; al 22/05/2026 e stato aggiunto il blueprint locale `docs/apiary_blueprint.txt` e al 25/05/2026 la specifica OpenAPI locale `docs/openapi.yaml`. Resta eventuale la pubblicazione su piattaforme esterne.
...
| Da consolidare | uniformazione risposte API, pubblicazione esterna Apiary/OpenAPI, test frontend/mobile |
...
- documentazione API completata localmente come Apiary Blueprint e OpenAPI, ma non ancora pubblicata su piattaforme esterne;
...
- gli endpoint legacy `/api/v1/scambi` sono stati deprecati e vanno spiegati come compatibilita non operativa.

Azioni per il prossimo sprint:

- pubblicare esternamente le specifiche API locali gia presenti in `docs/apiary_blueprint.txt` e `docs/openapi.yaml`;
...
Le funzionalita principali dell'applicazione sono esposte come API REST sotto il prefisso versionato `/api/v1`. La specifica Apiary locale e mantenuta in `docs/apiary_blueprint.txt`; la versione OpenAPI locale e mantenuta in `docs/openapi.yaml`.

| Metodo | Endpoint | Auth | Descrizione |
| --- | --- | --- | --- |
| POST | `/api/v1/auth/register` | No | Registra utente e crea wallet |
| POST | `/api/v1/auth/login` | No | Effettua login e restituisce JWT |
| POST | `/api/v1/auth/logout` | Si | Logout logico lato client |
| PUT | `/api/v1/users/me` | Si | Aggiorna profilo utente |
| GET | `/api/v1/users/:id/profilo` | No | Legge profilo pubblico |
| GET | `/api/v1/annunci` | Opzionale | Catalogo pubblico con filtri e paginazione |
| GET | `/api/v1/annunci/me` | Si | Annunci dell'utente autenticato |
| GET | `/api/v1/annunci/:id` | Opzionale | Dettaglio annuncio |
| POST | `/api/v1/annunci` | Si | Crea annuncio |
| PUT | `/api/v1/annunci/:id` | Si | Modifica annuncio |
| DELETE | `/api/v1/annunci/:id` | Si | Cancella logicamente annuncio |
| PATCH | `/api/v1/annunci/:id/stato` | Si | Cambia stato annuncio |
| POST | `/api/v1/prenotazioni` | Si | Crea prenotazione |
| GET | `/api/v1/prenotazioni/me` | Si | Lista prenotazioni dell'utente |
| GET | `/api/v1/prenotazioni/:id` | Si | Dettaglio prenotazione |
| DELETE | `/api/v1/prenotazioni/:id` | Si | Annulla prenotazione |
| POST | `/api/v1/prenotazioni/:id/no-show` | Si | Segnala mancato ritiro |
| POST | `/api/v1/prenotazioni/:id/disdici` | Si | Disdice ritiro |
| POST | `/api/v1/qr/genera` | Si | Genera QR per prenotazione |
| POST | `/api/v1/qr/valida` | Si | Valida QR e completa scambio |
| GET | `/api/v1/wallet/me` | Si | Saldo e storico in una risposta |
| GET | `/api/v1/wallet/saldo` | Si | Solo saldo |
| GET | `/api/v1/wallet/storico` | Si | Storico paginato e filtrabile |
| GET | `/api/v1/conversazioni/me` | Si | Lista conversazioni utente |
| GET | `/api/v1/conversazioni/me/non-letti` | Si | Conteggio messaggi non letti |
| GET | `/api/v1/conversazioni/:id/messaggi` | Si | Storico paginato conversazione |
| POST | `/api/v1/conversazioni/:id/messaggi` | Si | Invia messaggio in conversazione |
| GET | `/api/v1/messaggi/:prenotazioneId` | Si | Storico messaggi legacy per prenotazione |
| POST | `/api/v1/messaggi/:prenotazioneId` | Si | Invia messaggio legacy per prenotazione |
| PATCH | `/api/v1/messaggi/:id/letto` | Si | Marca messaggio come letto |
| GET | `/api/v1/notifiche/me` | Si | Lista notifiche utente |
| PATCH | `/api/v1/notifiche/me/leggi-tutte` | Si | Marca tutte le notifiche come lette |
| PATCH | `/api/v1/notifiche/:id/letta` | Si | Marca una notifica come letta |
| GET | `/api/v1/admin/statistiche` | Admin | Dashboard statistiche |
| GET | `/api/v1/admin/segnalazioni` | Admin | Lista segnalazioni |
| POST | `/api/v1/admin/utenti/:id/ban` | Admin | Banna utente |
| POST | `/api/v1/admin/utenti/:id/sospendi` | Admin | Sospende utente |
| POST | `/api/v1/admin/utenti/:id/riabilita` | Admin | Riabilita utente |
| PATCH | `/api/v1/admin/annunci/:id/forza` | Admin | Forza stato annuncio |
| DELETE | `/api/v1/admin/annunci/:id` | Admin | Rimuove annuncio |
| GET | `/api/v1/scambi/:prenotazioneId/qr` | No | Legacy deprecato, risponde `410 Gone` |
| POST | `/api/v1/scambi/:prenotazioneId/valida` | No | Legacy deprecato, risponde `410 Gone` |
...
Il report dichiara anche i limiti dello sprint: specifiche Apiary/OpenAPI presenti localmente ma non ancora pubblicate esternamente, test frontend/mobile assenti, servizi esterni D2 non implementati e recensioni ancora parziali. Questi elementi sono stati mantenuti nel product backlog per gli sprint successivi.

...
| Branch stabile | `main` |
| Apiary | Blueprint locale disponibile in `docs/apiary_blueprint.txt`; resta da pubblicare sulla piattaforma Apiary esterna |
...
| PB19 | Specifica Apiary/OpenAPI | Media | Apiary locale completato | `docs/apiary_blueprint.txt`; OpenAPI non generata |
...
La review ha inoltre confermato che tutte le funzionalita principali sono esposte tramite API REST, come richiesto dal template di milestone. La specifica Apiary/OpenAPI non era stata pubblicata nello Sprint 1; al 22/05/2026 e stato aggiunto il blueprint locale `docs/apiary_blueprint.txt`, da pubblicare eventualmente sulla piattaforma Apiary esterna.
...
- documentazione API completata come blueprint locale, ma non ancora pubblicata su Apiary esterno/OpenAPI;
...
- pubblicare esternamente o convertire in OpenAPI la specifica API gia presente in `docs/apiary_blueprint.txt`;
...
Le funzionalita principali dell'applicazione sono esposte come API REST sotto il prefisso versionato `/api/v1`. La specifica Apiary locale e mantenuta in `docs/apiary_blueprint.txt`.
...
Il report dichiara anche i limiti dello sprint: blueprint Apiary locale presente ma non ancora pubblicato esternamente o convertito in OpenAPI, test frontend/mobile assenti, servizi esterni D2 non implementati e recensioni ancora parziali. Questi elementi sono stati mantenuti nel product backlog per gli sprint successivi.
